...
void _dw_tree_node_set_data(DWTreeNode *self, guint index, gpointer data) {
    GList *list_elem;
    g_return_if_fail(DW_IS_TREE_NODE(self));
    /* Load the element we are looking for into list_elem */
    while(!(list_elem = g_list_nth(self->data_list, index)))
    {
        /* If the element doesn't exist append empty elements. */
        self->data_list = g_list_append(self->data_list, NULL);
    }
    /* Now that we have the element, replace the data */
    list_elem->data = data;
}
...
static void _dw_tree_node_setup_listitem_cb(GtkListItemFactory *factory, GtkListItem *list_item, gpointer user_data)
{
    ...
    GtkWidget *list_view = GTK_WIDGET(g_object_get_data(G_OBJECT(factory), "_dw_user"));
    ...
    /* Attach a GestureClick for right-click (Button 3) */
...
static void _dw_container_setup_cb(GtkListItemFactory *factory, GtkListItem *list_item, gpointer data)
{
    ...
    else if(flags & DW_CFA_BITMAPORICON)
        child = gtk_image_new();
    else
    {
    ...
    if(handlerdata)
    {
       ...
       if(work.window)
       {
           ...
           if(widget && GTK_IS_LIST_VIEW(widget))
           {
               ...
               gpointer item = g_list_model_get_item(G_LIST_MODEL(selection_model), selected);
...
static gint _dw_drop_drown_select_event(GObject *gobject, GParamSpec *pspec, gpointer data)
{
   ...
   if(work.window && GTK_IS_DROP_DOWN(widget))
   {
       ...
       if(selected != GTK_INVALID_LIST_POSITION)
       {
          ...
          retval = selectfunc(work.window, selected, work.data);
       }
   }
   return retval;
}
...
#if GTK_CHECK_VERSION(4,10,0) && !defined(DW_INCLUDE_DEPRECATED)
GListStore *_dw_list_model_get_store(GtkSelectionModel *list_model)
{
     if(list_model)
     {
         GListModel* selection_model = NULL;
         
         if(GTK_IS_SINGLE_SELECTION(list_model))
              selection_model = gtk_single_selection_get_model(GTK_SINGLE_SELECTION(list_model));
         else if(GTK_IS_MULTI_SELECTION(list_model))
              selection_model = gtk_multi_selection_get_model(GTK_MULTI_SELECTION(list_model));
         
         if(selection_model)
         {
            if(G_IS_LIST_STORE(selection_model))
            {
                return G_LIST_STORE(selection_model);
            }
         }
     }
     return NULL;
...
GListStore *_dw_container_get_store(GtkColumnView *cont)
{
     GtkSelectionModel *list_model = gtk_column_view_get_model(cont);
     return _dw_list_model_get_store(list_model);
}

GListStore *_dw_listbox_get_store(GtkListView *listbox)
{
     GtkSelectionModel *list_model = gtk_list_view_get_model(listbox);
     return _dw_list_model_get_store(list_model);
}
#endif
...
DW_FUNCTION_RESTORE_PARAM2(handle, HWND, rowcount, int)
{
   ...
   if(cont && GTK_IS_COLUMN_VIEW(cont))
   {
       ...
       if(store)
       {
          ...
          {
             g_list_store_append(store, _dw_tree_node_new());
          }
...
void _dw_container_set_item_int(HWND handle, void *pointer, int column, int row, void *data)
{
   ...
   if(cont && GTK_IS_COLUMN_VIEW(cont))
   {
       ...
       if(store)
       {
          ...
          if((node = DW_TREE_NODE(g_list_model_get_object(G_LIST_MODEL(store), row))))
          {
             ...
             {
                ULONG val = data ? *((ULONG *)data): 0;
                
                sprintf(textbuffer, "%lu", val);
...
DW_FUNCTION_RESTORE_PARAM3(handle, HWND, column, int, width, int)
{
   ...
   /* Make sure it is the correct tree type */
   if(cont && GTK_IS_COLUMN_VIEW(cont) &&
      g_object_get_data(G_OBJECT(cont), "_dw_tree_type") == GINT_TO_POINTER(_DW_TREE_TYPE_CONTAINER))
   {
        GListModel *columns_model = gtk_column_view_get_columns(GTK_COLUMN_VIEW(cont));
        GtkColumnViewColumn *columns = GTK_COLUMN_VIEW_COLUMN(g_list_model_get_item(columns_model, column));
        
        if(column && GTK_IS_COLUMN_VIEW_COLUMN(columns))
        {
            gtk_column_view_column_set_fixed_width(columns, width);
        }
   }
#else
   /* Make sure it is the correct tree type */
   if(cont && GTK_IS_TREE_VIEW(cont) &&
      g_object_get_data(G_OBJECT(cont), "_dw_tree_type") == GINT_TO_POINTER(_DW_TREE_TYPE_CONTAINER))
   {
   ...
   if(cont && GTK_IS_COLUMN_VIEW(cont))
   {
       ...
       if(store)
       {
            ...
            if(node)
            {
                ...
                else if(type == _DW_DATA_TYPE_POINTER)
                    _dw_tree_node_set_itemdata(node, data);
            }
       }
   }
...
DW_FUNCTION_RESTORE_PARAM3(handle, HWND, direction, int, rows, long)
{
   ...
   {
      GListStore *store = _dw_container_get_store(GTK_COLUMN_VIEW(cont));
      gint rowcount = store ? g_list_model_get_n_items(G_LIST_MODEL(store)) : 0;
#else
   if(cont && GTK_IS_TREE_VIEW(cont) &&
      g_object_get_data(G_OBJECT(cont), "_dw_tree_type") == GINT_TO_POINTER(_DW_TREE_TYPE_CONTAINER))
   {
       ...
       if(store)
       {
          ...
          if(GTK_IS_SELECTION_MODEL(selection_model))
          {
              if(flags & DW_CRA_SELECTED)
              {
                  ...
                  {
                      DWTreeNode *node = DW_TREE_NODE(g_list_model_get_object(G_LIST_MODEL(selection_model), value));
                      
                      g_object_set_data(G_OBJECT(cont), "_dw_querypos", GINT_TO_POINTER(value+1));
                      if(node)
                      {
              ...
              {
                  DWTreeNode *node = DW_TREE_NODE(g_list_model_get_object(G_LIST_MODEL(store), 0));
       ...
       if(store)
       {
          ...
          if(pos && GTK_IS_SELECTION_MODEL(selection_model))
          {
              if(flags & DW_CRA_SELECTED)
              {
                  ...
                  gtk_bitset_iter_init_at(&iter, selected_indexes, pos - 1, &value);
                  if(gtk_bitset_iter_is_valid(&iter))
              ...
              {
                  DWTreeNode *node = DW_TREE_NODE(g_list_model_get_object(G_LIST_MODEL(store), pos));
...
#if GTK_CHECK_VERSION(4,10,0) && !defined(DW_INCLUDE_DEPRECATED)
int _dw_find_node(GListStore *store, void *data, int textcomp)
{
   int z, rows = g_list_model_get_n_items(G_LIST_MODEL(store));
   int retval = -1;
   ...
   for(z=0;z<rows;z++)
   {
      ...
      if((node = DW_TREE_NODE(g_list_model_get_item(G_LIST_MODEL(store), z))))
      {
         if((textcomp && _dw_tree_node_get_name(node) && strcmp(_dw_tree_node_get_name(node), (char *)data) == 0) ||
            (!textcomp && _dw_tree_node_get_itemdata(node) == data))
         {
            retval = z;
            z = rows;
         }
...
void _dw_container_cursor_int(HWND handle, void *data, int textcomp)
{
      ...
      g_object_get_data(G_OBJECT(cont), "_dw_tree_type") == GINT_TO_POINTER(_DW_TREE_TYPE_CONTAINER))
   {
       ...
       if(store)
       {
            int index = _dw_find_node(store, data, textcomp);
            
            if(index != -1)
            {
                GtkSelectionModel *selection_model = gtk_column_view_get_model(GTK_COLUMN_VIEW(cont));

                if(GTK_IS_SINGLE_SELECTION(selection_model))
                {
                    /* Set the selected row by its index (0-based) */
...
GObject *_dw_tree_setup(struct _dw_signal_list *signal, GObject *object, void *sigfunc, void *discfunc, void *data)
{
   ...
   if(GTK_IS_LIST_VIEW(object))
   {
      ...
      else if(strcmp(signal->name, DW_SIGNAL_ITEM_CONTEXT) == 0)
      {
          ...
          * Since the handlers for the columns were already created in _dw_container/tree_setup()
          */
         int sigid = _dw_set_signal_handler(object, (HWND)object, sigfunc, data, signal->func, discfunc);
         g_object_set_data(object, "_dw_context_id", GINT_TO_POINTER(sigid+1));
         return NULL;
      }
#else
